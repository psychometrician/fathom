...
Scoring header follows ../../01-npm-registry/r/try-purrr.R, which is the template.
  ...
  measured      2026-08-11
  run           cd corpus/23-cratesio-summary/python && uv run try-glom.py
  ...
  13 needed the shape in advance?                    YES for 8, 9 — and for question 3
                                                     you must already suspect the four
                                                     collections are one shape
  14 survives the next file unchanged?               the hand walks do
  15 readable a week later?                          yes
  16 lines, and how much is ceremony?                ~120, glom on about 6

  THE DEFECT-25 DOCUMENT. glom can PROVE the four collections share a shape —
  one `set` of sorted key tuples — and volunteers nothing, which is this
  corpus's usual sentence. What is new here is the SECOND fact, which neither
  the tools nor the probe report: THE FOUR LISTS OVERLAP. 40 rows, 33 distinct
  crates, seven crates in two collections each.

  QUESTION 10 HAS NO TARGET. The deepest structure below the collections is
  `links`, an object of six fields — there is no array to flatten at all, and
  this is the first corpus document where that is true.
"""
import json
import re
import time
...
# ── Q8/Q9/Q10. ─────────────────────────────────────────────────────────────
t = time.time()
rows = glom(doc["new_crates"], [{"name": "name", "version": "max_version",
                                 "downloads": "downloads"}])
print(f"\nQ8  glom(new_crates, [{{...}}]) -> {len(rows)} rows x 3, {time.time()-t:.2f}s")
print(f"    {rows[0]}")
hp = glom(crates, [Coalesce("homepage", default="<absent>")])
...
print("    so `Coalesce` hands back the null. Entry 22 measured the same and")
print("    purrr's `pluck` doing the opposite; this is the third document to")
print("    separate the two behaviours.")
